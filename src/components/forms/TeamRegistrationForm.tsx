...
import { INITIAL, TEAM_STEPS } from "./form-types";
...
const TEAM_INITIAL: FullRegistrationFormData = { ...INITIAL, registrationType: "team" };

export function TeamRegistrationForm({ onBack }: { onBack: () => void }) {
  const [step, setStep]               = useState(0);
  const [data, setData]               = useState<FullRegistrationFormData>(TEAM_INITIAL);
  ...
  function runValidation(s: number): FormErrors {
    const schema = getStepSchema(s, "team");
    if (!schema) return {};
  ...
  return (
    <form
      ...
      dir="rtl"
    >
      {step > 0 && <StepProgress current={step} steps={TEAM_STEPS} />}
