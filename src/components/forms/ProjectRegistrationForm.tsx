...
import { INITIAL, PROJECT_STEPS } from "./form-types";
...
import { Step5Project } from "./steps/Step5Project";
import { Step7 } from "./steps/Step7";

// Project form: steps 0(preamble)→1→2→3→4→5(project select)→6(conclusion)
const LAST_STEP = 6;
const PROJECT_INITIAL: FullRegistrationFormData = { ...INITIAL, registrationType: "project" };

export function ProjectRegistrationForm({ onBack }: { onBack: () => void }) {
  const [step, setStep]               = useState(0);
  const [data, setData]               = useState<FullRegistrationFormData>(PROJECT_INITIAL);
  ...
  function runValidation(s: number): FormErrors {
    const schema = getStepSchema(s, "project");
    if (!schema) return {};
  ...
  function goNext() {
    ...
    setErrors({});
    setSubmitError(null);
    setStep(s => (s === 0 ? 1 : Math.min(LAST_STEP, s + 1)));
  }
  ...
  async function submit() {
    const errs = runValidation(LAST_STEP);
    if (Object.keys(errs).length > 0) {
      setErrors(errs);
      setTimeout(() => {
        document.querySelector("[role=alert]")?.scrollIntoView({ behavior: "smooth", block: "center" });
    ...
    return (
      <div className="text-center py-12 space-y-4">
        ...
        <p className="text-on-surface-variant max-w-sm mx-auto">
          سيتم مراجعة استمارتك والتواصل معك قريباً. شكراً لاهتمامك بالانضمام إلى مشاريع أكاديمية جيل الترجيح.
        </p>
  ...
  return (
    <form
      ...
      dir="rtl"
    >
      {step > 0 && <StepProgress current={step} steps={PROJECT_STEPS} />}
      ...
      <div>
        {step === 0 && <StepPreamble onStart={goNext} type="project" />}
        {step === 1 && <Step1 {...stepProps} />}
        {step === 2 && <Step2 {...stepProps} />}
        {step === 3 && <Step3 {...stepProps} />}
        {step === 4 && <Step4 {...stepProps} />}
        {step === 5 && <Step5Project {...stepProps} />}
