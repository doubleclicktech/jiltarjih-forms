...
const STATS = [
  { value: "+120", label: "مشاركًا فاعلًا" },
  { value: "10", label: "فرق عمل" },
  { value: "5", label: "مشاريع فاعلة" },
];
  ...
  return (
    <section className="relative isolate overflow-hidden min-h-screen flex flex-col">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-center w-full">
          <div className="lg:col-span-3 text-right space-y-7">
            ...
            <div className="inline-flex items-center gap-2.5 rounded-full border border-secondary/30 bg-secondary/10 backdrop-blur-sm px-5 py-2.5">
              ...
              <span className="text-sm font-semibold text-secondary tracking-wide">منصّة المشاريع والفرق</span>
            </div>

            <div>
              <h1 className="leading-[1.15]" style={{ fontFamily: "var(--font-cairo), Cairo, sans-serif" }}>
                <span className="block text-white/60 font-medium mb-1" style={{ fontSize: "clamp(1.1rem, 2.5vw, 1.5rem)" }}>
                  منارة
                </span>
                <span className="block text-white font-black" style={{ fontSize: "clamp(2.4rem, 5.5vw, 4.2rem)" }}>
                  نبني المؤسسات
                </span>
                <span className="block font-black" style={{ fontSize: "clamp(2.4rem, 5.5vw, 4.2rem)", color: "#ECB12E" }}>
                  نصنع الأثر
                </span>
              </h1>
            ...
            <div className="space-y-3">
              <p className="text-white/65 leading-loose max-w-xl" style={{ fontSize: "clamp(0.95rem, 1.4vw, 1.1rem)" }}>
                بوّابة المشاريع وفرق العمل في أكاديمية جيل الترجيح، ترافق انتقالها من الفكرة والنواة الأولى إلى مشروع منظّم ومؤسسة ناضجة ذات أثر حضاري.
              </p>
              <p className="text-secondary/90 font-semibold">الفكرة ← النواة ← فريق العمل ← المشروع ← المؤسسة ← الأثر.</p>
              <p className="text-white/80 font-bold">نحو شبكةٍ تقودها 100 مؤسسة أنموذجيّة على الأقل خلال 15 سنة.</p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-start gap-3">
              ...
              >
                انضم الآن
                <span className="material-symbols-outlined text-lg transition-transform group-hover:-translate-x-1" style={{ fontVariationSettings: '"FILL" 1' }}>
                  arrow_back
                </span>
              </Link>
              <Link
            ...
            <div className="flex justify-start gap-6 sm:gap-10 pt-5 border-t border-white/[0.08]">
              {STATS.map((s) => (
                <div key={s.label} className="text-right">
                  <p className="font-black text-secondary leading-none" style={{ fontSize: "clamp(1.5rem, 2.8vw, 2rem)" }}>
                    {s.value}
                  </p>
      ...
      <div className="relative z-10 flex flex-col items-center gap-1.5 pb-8 text-white/30">
        <span className="text-[10px] tracking-[0.25em] uppercase">اكتشف منارة</span>
        <span className="material-symbols-outlined text-xl animate-bounce" style={{ fontVariationSettings: '"FILL" 0, "wght" 200' }}>
          keyboard_arrow_down
        </span>
      </div>
    </section>
